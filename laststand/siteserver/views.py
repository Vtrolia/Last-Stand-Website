...
def submit_application(request):
    ...
    mail = EmailMessage(request.user.get_username() + " wants to become a Publisher!",
                        "This is what they had to say:\n\n" + request.POST['introduction'], to=["vinny@mail.laststandcloud.com"])
...
def submit_change(request):
    to_change = j.loads(request.body.decode("utf-8"))
    result = {"flag": "", "body": ""}
...
def load_articles(request):
    ...
    article = Articles.objects.filter(confirmed=True).order_by("-id")[request.session["offset"]: request.session["offset"] + 5]
    articles = {}
